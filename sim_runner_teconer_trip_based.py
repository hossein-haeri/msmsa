...
def run(online_model, dataset_name, preview_duration, run_name=None):
    ...
    data_y_pred = np.zeros(len(data_y))
    data_y_pred[:] = np.nan
...
if base_learner_name == 'RF':
    online_model.base_learner = RandomForestRegressor(n_estimators=50, max_depth=7, n_jobs=4, bootstrap=True, max_samples=.9, min_samples_leaf=5)
...
if wandb_log:
        ...
        summary = run(online_model=online_model, dataset_name=dataset_name, preview_duration=preview_duration, run_name=run_name)
else:
    summary = run(online_model=online_model, dataset_name=dataset_name, preview_duration=preview_duration)
